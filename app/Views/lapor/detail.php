...
<div class="detail-card">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <?php
            $statusClass = 'status-' . strtolower($laporan['status']);
        ?>
        <span class="status-badge <?= $statusClass ?>">
            <?= ucfirst(esc($laporan['status'])) ?>
        </span>
    </div>
    ...
    <dl class="detail-grid">
        <dt>Nama Pelapor</dt>
        <dd><?= esc($laporan['nama_pelapor']) ?></dd>
        ...
        <dt>Kategori</dt>
        <dd><?= esc($laporan['kategori']) ?></dd>

        <dt>Prioritas</dt>
        <dd><?= esc($laporan['prioritas']) ?></dd>
    </dl>
    ...
    <div class="photo-gallery">
        <?php
            $foto_files = json_decode($laporan['foto'], true);
            if (!empty($foto_files)):
    ...
    <div class="mt-4 text-center">
        <a href="<?= site_url('lapor/status') ?>" class="btn btn-secondary">
